...
const Roles: React.FC<RolesProps> = ({
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                ...
                recruiterStore.setState({ roles: fetchedData });
                setData(fetchedData);
            } catch (error) {
    ...
    const onSubmit = async (data: FormValues) => {
        try {
            recruiterStore.setState({ selectedRoles: data.roles });
            // console.log(recruiterStore.getState().selectedRoles);
        } catch (error: any) {
    ...
    return (
        <>
            ...
            2xl:w-[580px]
            2xl:mb-[18px]
            2xl:ml-[20px]">Select the roles that align best with your interests, knowledge, experience, and wishes. </p>
            <Form {...form}>
                                            ...
                                            render={({ field }) => {
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <FormLabel className={`border-2 rounded-[40px] !mt-0 px-[30px] py-[14px]
                                                        bg-[#fff] border-[#5496EE] hover:border-[#58D336] hover:text-[#58D336] cursor-pointer font-medium text-[18px] ${field.value?.includes(item.id.toString()) ? "text-[#ffffff] bg-gradient-to-br from-[#58D336] to-[#3E9626] border-[#ffffff] hover:text-[#ffffff]" : "text-[#5496EE]"}`}>
                                                            {item.name}
                                                        </FormLabel>
                                                    </FormItem>
                                                )
                    ...
                    <div className="flex justify-between items-center pt-8">
                        ...
                        <div className="flex flex-row justify-center items-center mx-auto py-6">
                            <p className="text-[22px] font-medium tracking-[5px]">
                                <span className="text-[#5496EE]">3</span>/5
                            </p>
                        </div>
